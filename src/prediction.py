...
def _load_model_and_meta():
    ...
    try:
        model = tf.keras.models.load_model(
            MODEL_PATH,
            custom_objects={"Functional": tf.keras.Model}  # patch untuk TF2.15 ↔ Keras3
        )
    ...
    if not has_tv:
        st.error("Model tidak berisi layer TextVectorization.")
        st.stop()
...
def _predict_texts(model, texts: List[str], id2sent: Dict[int, str]) -> pd.DataFrame:
    ...
    if probs.shape[1] != 3:
        raise ValueError(f"Model mengeluarkan {probs.shape[1]} kelas; diharapkan 3.")
    yhat = probs.argmax(axis=1)
    pred_sent = [id2sent.get(int(i), str(i)) for i in yhat]
    return pd.DataFrame({
        "pred_sent": pred_sent,
        "p_neg": probs[:, 0],
        "p_neu": probs[:, 1],
    ...
    with tabs[1]:
        ...
        if file is not None:
            try:
                ...
                return
            if df.empty:
                st.warning("CSV kosong."); return
            text_col = _detect_text_col(df)
            if not text_col:
                st.error("Tidak menemukan kolom teks."); return
